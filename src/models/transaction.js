...
//     return await pool.query('select * from transactions INNER JOIN transaction_item ON transactions.transaction_id  = transaction_item.transaction_id INNER JOIN items on items.item_id = transaction_item.item_id INNER JOIN quantity_location ON transaction_item.qyt_loc_id = quantity_location.qyt_loc_id INNER JOIN waybill ON transactions.waybill_id = waybill.waybill_id where transactions.created_by_id=? OR waybill.sent_to_id=? GROUP BY transactions.transaction_id ORDER BY transactionDate DESC LIMIT ? OFFSET ?', [userId, userId, limit, offset])
...
console.log(date_in_loc)
 ...
 export const findTransactionById = async (transId) => {
   return await pool.query('select * from transactions where transaction_id=? ', [transId])
} 